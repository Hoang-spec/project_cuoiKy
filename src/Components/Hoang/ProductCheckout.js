...
function ProductCheckout() {
  ...
  useEffect(() => {
    ...
    console.log(user)
    if (user) {
      setValue('name', user.name || '');  // Điền tên người dùng
      setValue('phone', user.phoneNumber || '');  // Điền số điện thoại
      setValue('email', user.email || '');  // Điền email
    }
  }, [setValue]);
  ...
  return (
    <div className="Checkout">
      <h1 style={{ color: 'black', marginTop: '10px' }}>CHECK OUT</h1>
      <form onSubmit={handleSubmit(xong)}>
        <main>
          <div className="section">
            <section className="Checkout_section1">
              <h1 style={{ fontSize: '18px', color: 'black', marginTop: '10px' }}>THÔNG TIN</h1>
              <input 
                type="text" 
            ...
            <section className="Checkout_section2">
              <h1 style={{ fontSize: '18px', color: 'black', marginTop: '10px' }}>HÌNH THỨC NHẬN HÀNG</h1>
              <input 
                type="radio" 
              ...
              {errors.deliveryMethod && <span style={{ color: "red", margin: '60px' }}>{errors.deliveryMethod.message}</span>}
            ...
            <section className="Checkout_section3">
              <h2 style={{ fontSize: '18px', color: 'black', marginTop: '10px' }}>PHƯƠNG THỨC THANH TOÁN</h2>
          ...
          <section className="section_total">
            <h1 style={{ fontSize: '18px', color: 'black' }}>TỔNG TIỀN</h1>
